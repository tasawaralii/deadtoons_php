...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (isset($_POST['action']) && $_POST['action'] === "reply") {
        ...
        $reply = $_POST['reply'] ?? '';
        $par_id = $_POST['com_par'] ?? 0;
        $modKey = $_POST['moderator'] ?? '';

        $moderators = [
            "babyxboss" => [
                "name" => "BabyxBoss",
                "gmail" => "babyxboss0@gmail.com",
                "site" => "/"
            ],
            "admin" => [
                "name" => "Admin",
                "gmail" => "zikdeathnote@gmail.com",
                "site" => "/"
            ]
        ];

        // Validate moderator
        if (!isset($moderators[$modKey])) {
            die("Invalid moderator");
        }

        $mod = $moderators[$modKey];

        $sql = "INSERT INTO comments 
                (post_id, parent_id, com_author, com_email, com_author_url, com_date, com_content, com_status)
                VALUES 
                (:id, :par_id, :moder_name, :moder_gmail, :moder_site, NOW(), :reply, 1)";
        ...
        <td id="reply_box" colspan="7">
            <form method="post">
                <div class="form-group">
                    ...
                    <label for="moderator-select">Choose a Moderator:</label>
                    <select name="moderator" id="moderator-select">
                        <option value="babyxboss">BabyxBoss</option>
            ...
            <tbody>
                <?php foreach ($comments as $comment): ?>
                    <tr id="<?php echo $comment['com_id']; ?>">
                        ...
                        <td>
                            <a href="edit-comment.php?id=<?php echo $comment['com_id']; ?>"
                                class="btn btn-primary btn-sm">Edit</a>
                            <button class="btn btn-danger btn-sm"
                                onclick="delete_com(<?php echo $comment['com_id'] ?>)">Delete</button>
                            <button class="btn btn-info btn-sm"
                                onclick="reply_com(<?php echo $comment['id'] . ',' . $comment['com_id'] ?>)">Reply</button>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
